...
%=============================================================================
%
function sys=mdlUpdate(t,x,u,flag,encoder_ppv)
%u(1)= Va Sensor A
%u(2)= Vb Sensor B
%x   = Velocidad angular


%if u(1) == 0, %Flanco descendente
%	x=2*pi/T*encoder_ppv; %Calculo el angulo
%end
%sys=x;
	sys(3)=;%Arranco en cero.
	estado=u(1)+10*u(2); % Codificacion del grafo, 00 01 10 11
	if(estado~= x(1)),	%por cada 
		sys(1)=estado;
		sys(3)=sys(2)
		sys(2)=0;
	else
...
%x(3) = tiempo acumulado desde el flanco ascendente
sys=(pi/2)*x(3)/(x(2)*Ts)
